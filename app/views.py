...
def signin(request):
    if EmailValidation.objects.filter(
        ...
        if present_day.day != 1:
            ...
            empty_list = []
            if Leavebalance.objects.filter(leavetype__leavetype='Loss of pay').filter(emp__email=request.user.email).exists():
...
@permission_required("app.change_customuser", login_url='/accounts/google/login/')
...
@permission_required("app.add_rolepermission", login_url='/accounts/google/login/')
def addnewstatus(request):
    if request.method == "POST":
        ...
        messages.info(request, "Role Added Successfully with Priviliges.")
        return redirect(addnewstatus)
    groups = Group.objects.all()
    context = {
        'groups':groups,
    }
...
def addemp(request):
    ...
    if request.method == "POST":
        ...
        if EmailValidation.objects.filter(username=username,email=email).exists():
            ...
            for x in leavetypes:
                Leavebalance.objects.create(emp=data, leavetype=x, alloated=21, balance=21)
...
def empholidayview(request):
    ...
    holidays = AllHolidays.objects.filter(region = reg)
    context = {
        "holidays": holidays,
    }
...
def attendance(request,year=now.year,month=now.month):
    ...
    total=2025
    email=request.user.email
    region1 = EmailValidation.objects.filter(email=email).values_list('region')
    region2=[t[0] for t in region1 ]
    region=int(region2[0])
...
@permission_required('app.add_leavetypepermission', login_url='/accounts/google/login/') 
